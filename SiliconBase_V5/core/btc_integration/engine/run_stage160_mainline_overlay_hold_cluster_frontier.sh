...
"$PY" -u -m tools.stage160_mainline_overlay_hold_cluster_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage160_mainline_overlay_hold_cluster_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
if not files:
    ...
    empty.write_text('stage160 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage160_mainline_overlay_hold_cluster_frontier_done"
