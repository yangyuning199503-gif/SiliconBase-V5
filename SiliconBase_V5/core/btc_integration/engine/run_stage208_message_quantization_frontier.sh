...
OUT_ZIP="$DOWNLOADS/stage208_message_quantization_frontier_latest.zip"
TMP_DIR="$ROOT/reports/research_raw/stage208_message_quantization_frontier_tmp"
LOG="$TMP_DIR/stage208_full_log.txt"
STATUS="$TMP_DIR/stage208_step_status.tsv"
SUMMARY="$TMP_DIR/stage208_message_quantization_frontier_bundle_latest.txt"
...
if ! run_step py_compile "$PY" -m py_compile \
  "$ROOT/tools/stage208_message_quantization_frontier.py" \
...
if [ "$system_gate" = "PASS" ]; then
  ...
  if ! run_step stage208_message_quantization_frontier "$PY" -u "$ROOT/tools/stage208_message_quantization_frontier.py" --project-dir "$ROOT"; then
    overall_status="PARTIAL_FAIL"
  fi
else
  printf '%s\tSKIP\n' ensure_current_mainline_trades >> "$STATUS"
  printf '%s\tSKIP\n' stage208_message_quantization_frontier >> "$STATUS"
  echo "[$(date '+%F %T')] SKIP  stage208 because system_gate=FAIL" >> "$LOG"
fi

for f in \
  "$ROOT/reports/research_raw/stage208_message_quantization_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage208_message_quantization_frontier_latest.json" \
...
cat > "$SUMMARY" <<EOF2
Stage208 message quantization frontier bundle

- 对外只生成 1 个文件：stage208_message_quantization_frontier_latest.zip
- 先刷新当前主线 baseline trades，再做消息标准化权重前沿
- 不再用统一消息规则一刀切；消息分值 = 严重度 × 事件模式 × 币种映射 × 时效衰减 × 衍生品确认
...
- 重点不是拦更多单，而是少误杀：在接近最优收益时，优先选 blocked 更少的消息权重方案。
- 中间输出只写到 reports/research_raw；Downloads 只留 stage208 这一份 zip。
EOF2
