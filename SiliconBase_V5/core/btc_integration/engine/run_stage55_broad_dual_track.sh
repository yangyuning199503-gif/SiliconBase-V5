...
needs_install() {
  "$PY" - <<'PY' >/dev/null 2>&1
import importlib
for name in ["pandas", "requests", "yaml", "numpy"]:
...
"$PY" -m tools.stage55_broad_dual_track --project-dir .
"$PY" -m tools.stage55_pack_outputs --project-dir . --started-at "$START_EPOCH"

echo "[OK] stage55 done | outputs refreshed: ~/Downloads/deepseek_single_file_latest.txt + ~/Downloads/chatgpt_bundle_latest.zip"
