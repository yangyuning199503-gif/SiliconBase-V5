...
"$PY" -u -m tools.stage140_regime_switch_risk_ladder --project-dir "$ROOT"

OUT="$HOME/Downloads/stage140_regime_switch_risk_ladder_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage140_regime_switch_risk_ladder_latest.txt" \
  "$ROOT/reports/research_raw/stage140_regime_switch_risk_ladder_latest.json" \
  "$ROOT/reports/research_raw/stage140_regime_switch_risk_ladder_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage140 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage140_regime_switch_risk_ladder_done"
