...
if [ -z "$TRADES_PATH" ] || [ ! -f "$TRADES_PATH" ]; then
  RUN_ID="stage131_$(date +%Y%m%d_%H%M%S)"
...
OUT="$HOME/Downloads/stage131_bidirectional_guard_refresh_latest.zip"
TMPDIR="$(mktemp -d)"
...
if not files:
    ...
    empty.write_text('stage131 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage131_bidirectional_guard_refresh_done"
