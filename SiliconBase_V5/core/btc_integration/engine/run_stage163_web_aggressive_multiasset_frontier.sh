...
"$PY" -m py_compile \
  ...
  "$ROOT/tools/stage163_web_aggressive_multiasset_frontier.py"

"$PY" -u -m tools.stage163_web_aggressive_multiasset_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage163_web_aggressive_multiasset_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage163_web_aggressive_multiasset_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage163_web_aggressive_multiasset_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage163_web_aggressive_multiasset_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage163 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage163_web_aggressive_multiasset_frontier_done"
