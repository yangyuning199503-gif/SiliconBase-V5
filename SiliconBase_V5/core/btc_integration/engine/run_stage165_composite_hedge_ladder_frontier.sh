...
"$PY" -u -m tools.stage165_composite_hedge_ladder_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage165_composite_hedge_ladder_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage165_composite_hedge_ladder_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage165_composite_hedge_ladder_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage165_composite_hedge_ladder_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage165 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage165_composite_hedge_ladder_frontier_done"
