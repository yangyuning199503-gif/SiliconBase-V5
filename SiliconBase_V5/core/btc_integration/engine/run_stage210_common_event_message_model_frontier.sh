...
OUT_ZIP="$DOWNLOADS/stage210_common_event_message_model_frontier_latest.zip"
TMP_DIR="$ROOT/reports/research_raw/stage210_common_event_message_model_frontier_tmp"
LOG="$TMP_DIR/stage210_full_log.txt"
STATUS="$TMP_DIR/stage210_step_status.tsv"
SUMMARY="$TMP_DIR/stage210_common_event_message_model_frontier_bundle_latest.txt"
...
# Optional refresh: do not fail the whole run if CoinGlass is temporarily unavailable.
...
if [ "$system_gate" = "PASS" ]; then
  ...
  if ! run_step stage210_common_event_message_model_frontier "$PY" -u "$ROOT/tools/stage210_common_event_message_model_frontier.py" --project-dir "$ROOT"; then
    overall_status="PARTIAL_FAIL"
  fi
else
  printf '%s\tSKIP\n' ensure_current_mainline_trades >> "$STATUS"
  printf '%s\tSKIP\n' stage210_common_event_message_model_frontier >> "$STATUS"
  echo "[$(date '+%F %T')] SKIP  stage210 because system_gate=FAIL" >> "$LOG"
fi

for f in \
  "$ROOT/reports/research_raw/stage210_common_event_message_model_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage210_common_event_message_model_frontier_latest.json" \
...
cat > "$SUMMARY" <<EOF2
Stage210 common event + message model frontier bundle

- 对外只生成 1 个文件：stage210_common_event_message_model_frontier_latest.zip
- 这版把“未来事件标准化 + 消息量化 + 行动分层”钉成统一公共模型。
...
[notes]
- 这版只动 research，不动 demo runtime。
- 重点不是继续抖统一阈值，而是先把 common model 做对：technical alpha / scheduled event / message score / derivatives confirm / action ladder。
- 中间输出只写到 reports/research_raw；Downloads 只留 stage210 这一份 zip。
EOF2
