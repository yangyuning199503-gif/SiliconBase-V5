...
"$PY" -m py_compile \
  "$ROOT/tools/stage135_web_playbook_frontier.py" \
  "$ROOT/tools/stage90_event_alpha_matrix.py" \
  "$ROOT/tools/stage88_strategy_fusion_walkforward.py"

"$PY" -u -m tools.stage135_web_playbook_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage135_web_playbook_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage135_web_playbook_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage135_web_playbook_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage135_web_playbook_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage135 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage135_web_playbook_frontier_done"
