...
"$PY" -u -m tools.stage142_reclaim_cluster_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage142_reclaim_cluster_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage142_reclaim_cluster_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage142_reclaim_cluster_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage142_reclaim_cluster_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage142 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage142_reclaim_cluster_frontier_done"
