...
"$PY" -m py_compile \
  "$ROOT/tools/stage134_target_monthly_frontier.py" \
  "$ROOT/tools/stage90_event_alpha_matrix.py" \
  "$ROOT/tools/stage120_event_window_frontier.py"

"$PY" -u -m tools.stage134_target_monthly_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage134_target_monthly_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage134_target_monthly_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage134_target_monthly_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage134_target_monthly_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage134 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage134_target_monthly_frontier_done"
