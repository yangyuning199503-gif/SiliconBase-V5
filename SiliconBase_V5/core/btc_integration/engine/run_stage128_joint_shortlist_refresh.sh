...
if [ -z "$TRADES_PATH" ] || [ ! -f "$TRADES_PATH" ]; then
  RUN_ID="stage128_$(date +%Y%m%d_%H%M%S)"
...
OUT="$HOME/Downloads/stage128_joint_shortlist_refresh_latest.zip"
TMPDIR="$(mktemp -d)"
...
if not files:
    ...
    empty.write_text('stage128 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage128_joint_shortlist_refresh_done"
