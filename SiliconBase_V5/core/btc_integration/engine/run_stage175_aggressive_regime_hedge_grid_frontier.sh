...
"$PY" -m py_compile \
  ...
  "$ROOT/tools/stage175_aggressive_regime_hedge_grid_frontier.py"

"$PY" -u -m tools.stage175_aggressive_regime_hedge_grid_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage175_aggressive_regime_hedge_grid_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage175_aggressive_regime_hedge_grid_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage175_aggressive_regime_hedge_grid_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage175_aggressive_regime_hedge_grid_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage175 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage175_aggressive_regime_hedge_grid_frontier_done"
