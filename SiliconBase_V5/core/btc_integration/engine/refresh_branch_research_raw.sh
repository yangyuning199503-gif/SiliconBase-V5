...
status=0
fetch_one ETHUSDT data/raw/eth_15m.csv || status=1
fetch_one SOLUSDT data/raw/sol_15m.csv || status=1

"$PY" - <<'PY' > reports/research_raw/branch_raw_freshness_latest.txt
...
for sym, path in paths.items():
    ...
    rows.append(f"- {sym.upper()}: latest={latest.strftime('%Y-%m-%d %H:%M:%S')} | rows={len(df)} | file={path}")
...
echo "[OK] reports/research_raw/branch_raw_freshness_latest.txt"
if [ "$status" -ne 0 ]; then
  echo "[WARN] 部分分支 raw 刷新失败；已尽量保留旧文件。"
fi
