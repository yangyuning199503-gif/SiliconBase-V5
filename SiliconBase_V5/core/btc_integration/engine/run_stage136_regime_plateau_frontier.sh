...
"$PY" -u -m tools.stage136_regime_plateau_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage136_regime_plateau_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage136_regime_plateau_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage136_regime_plateau_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage136_regime_plateau_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage136 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage136_regime_plateau_frontier_done"
