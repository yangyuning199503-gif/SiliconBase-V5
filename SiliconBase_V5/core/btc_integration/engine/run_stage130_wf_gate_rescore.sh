...
"$PY" -m py_compile \
  "$ROOT/tools/stage81_mainline_walkforward_lab.py" \
  "$ROOT/tools/stage82_branch_walkforward_lab.py" \
...
if [ -z "$TRADES_PATH" ] || [ ! -f "$TRADES_PATH" ]; then
  RUN_ID="stage130_$(date +%Y%m%d_%H%M%S)"
...
OUT="$HOME/Downloads/stage130_wf_gate_rescore_latest.zip"
TMPDIR="$(mktemp -d)"
...
if not files:
    ...
    empty.write_text('stage130 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage130_wf_gate_rescore_done"
