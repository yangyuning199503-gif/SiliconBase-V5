...
def _active_bias(row: dict[str, Any], active_map: dict[str, str], *, branch: bool) -> float:
    ...
    if sym == "ETH":
        if "reclaim_pair" in active and pb in {"eth_reclaim_pair_dense_fast", "eth_reclaim_pair_dense_core", "eth_reclaim_ladder_dense"}:
            return 220.0
        if "reclaim_pair" in active and pb == "eth_jump_reversal_dense":
            return 40.0
    if sym == "BTC":
        if "squeeze_follow" in active and pb == "btc_squeeze_follow_dense":
            return 120.0
        if "squeeze_follow" in active and pb == "btc_breakout_dense":
            return 80.0
    if sym == "SOL":
        if "pullback_pair" in active and pb in {"sol_pullback_pair_dense_fast", "sol_pullback_pair_dense_core"}:
            return 160.0
        if "pullback_pair" in active and pb == "sol_guarded_short_dense":
            return 35.0
...
def _score(row: dict[str, Any], runtime_anchor: dict[str, Any], active_map: dict[str, str], *, branch: bool) -> float:
    ...
    if not branch:
        if pb == "base":
            score += 80.0
        if pb == "mainline_bnb_split_reentry_dense":
            score += 200.0
        if best_m < 0.008:
            score -= 320.0
    ...
    if sym == "ETH":
        if pb == "eth_reclaim_pair_dense_fast":
            score += 360.0
        if pb == "eth_reclaim_pair_dense_core":
            score += 320.0
        if pb == "eth_reclaim_ladder_dense":
            score += 230.0
        if pb == "eth_squeeze_follow_dense":
            score += 120.0
        if pb == "eth_jump_reversal_dense":
            score += 80.0
        if pb in {"eth_reclaim_pair_dense_fast", "eth_reclaim_pair_dense_core", "eth_reclaim_ladder_dense", "eth_squeeze_follow_dense"}:
    ...
    if sym == "BTC":
        if pb == "btc_squeeze_follow_dense":
            score += 170.0
        if pb == "btc_breakout_dense":
            score += 150.0
        if pb == "btc_jump_reversal_dense":
            score += 60.0
        if pb in {"btc_squeeze_follow_dense", "btc_breakout_dense"}:
    ...
    if sym == "SOL":
        if pb == "sol_pullback_pair_dense_fast":
            score += 220.0
        if pb == "sol_pullback_pair_dense_core":
            score += 180.0
        if pb == "sol_range_ladder_dense":
            score += 90.0
        if pb == "sol_guarded_short_dense":
            score += 70.0
        if pb in {"sol_pullback_pair_dense_fast", "sol_pullback_pair_dense_core", "sol_range_ladder_dense"}:
...
def _mainline_items(existing_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    add(
        anchor,
        name="mainline_live_dynlev_fix8_lock18_bnb_split_reentry_lb21_buf043_cd5_bnb124_btc530",
        note="主线 stage193：只保 1 条 BNB split-reentry，高频但不再扩散扫面。",
        patch={
            "strategy_params.breakout_lookback": 21,
            ...
            "money_management.stake_scale.btc_short": 5.30,
        ...
        track="mainline_bnb_split_reentry_dense",
        playbook="mainline_bnb_split_reentry_dense",
    )
    return out
...
def _branch_items(existing_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    eth_ladder = item_map.get("eth_reclaim_ladder_long_lb10_atr040_adx15_cd1_s068") or item_map.get("eth_reclaim_ladder_long_lb11_atr043_adx16_cd2_s064") or eth_reclaim_pair
    ...
    add(
        ...
        note="BTC stage193：围绕当前 active 做一档更密的 squeeze 中速邻域。",
        family="long",
        patch={
        ...
        track="btc_squeeze_follow_dense",
        anchor_name=_name(btc_squeeze),
        playbook="btc_squeeze_follow_dense",
    )
    add(
        btc_breakout,
        name="btc_breakout_long_event_lb19_atr058_adx23_s052",
        note="BTC stage193：breakout 只留一条更贴近活跃锚点的 moderate 线。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 19,
        ...
        track="btc_breakout_dense",
        anchor_name=_name(btc_breakout),
        playbook="btc_breakout_dense",
    )
    add(
        btc_short,
        name="btc_retest_short_event_lb15_atr052_adx19_s076",
        note="BTC stage193：空腿只留一条 moderate retest。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 15,
        ...
        track="btc_jump_reversal_dense",
        anchor_name=_name(btc_short),
        playbook="btc_jump_reversal_dense",
    )

    add(
        eth_reclaim_pair,
        name="eth_reclaim_pair_long_lb9_atr038_adx14_cd1_s068",
        note="ETH stage193：围绕新 active 做更快一档 reclaim_pair。",
        family="long",
        patch={
        ...
        track="eth_reclaim_pair_dense_fast",
        anchor_name=_name(eth_reclaim_pair),
        playbook="eth_reclaim_pair_dense_fast",
    )
    add(
        eth_reclaim_pair,
        name="eth_reclaim_pair_long_lb11_atr042_adx16_cd1_s060",
        note="ETH stage193：围绕新 active 做更稳一档 reclaim_pair。",
        family="long",
        patch={
        ...
        track="eth_reclaim_pair_dense_core",
        anchor_name=_name(eth_reclaim_pair),
        playbook="eth_reclaim_pair_dense_core",
    )
    add(
        eth_ladder,
        name="eth_reclaim_ladder_long_lb9_atr038_adx14_cd1_s072",
        note="ETH stage193：ladder 只保一条更快邻域。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 9,
            "strategy_params.breakout_atr_buffer": 0.38,
        ...
        track="eth_reclaim_ladder_dense",
        anchor_name=_name(eth_ladder),
        playbook="eth_reclaim_ladder_dense",
    )
    add(
        eth_squeeze,
        name="eth_squeeze_follow_long_lb8_atr038_adx14_s056",
        note="ETH stage193：squeeze 继续只留一条补位。",
        family="long",
        patch={
        ...
        track="eth_squeeze_follow_dense",
        anchor_name=_name(eth_squeeze),
        playbook="eth_squeeze_follow_dense",
    )
    add(
        eth_short,
        name="eth_retest_short_trend_lb11_atr043_adx15_cd1_s078",
        note="ETH stage193：空腿只留一条更紧的 jump-reversal。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 11,
            "strategy_params.breakout_atr_buffer": 0.43,
        ...
        track="eth_jump_reversal_dense",
        anchor_name=_name(eth_short),
        playbook="eth_jump_reversal_dense",
    )

    add(
        sol_pullback,
        name="sol_pullback_pair_long_adx22_cd3_lb18_zone022_s052",
        note="SOL stage193：pullback_pair 更快一档，围绕新 long_watch 锚点密集扩圈。",
        family="long",
        patch={
        ...
        track="sol_pullback_pair_dense_fast",
        anchor_name=_name(sol_pullback),
        playbook="sol_pullback_pair_dense_fast",
    )
    add(
        sol_pullback,
        name="sol_pullback_pair_long_adx26_cd5_lb22_zone026_s044",
        note="SOL stage193：pullback_pair 更稳一档，保样本厚度。",
        family="long",
        patch={
        ...
        track="sol_pullback_pair_dense_core",
        anchor_name=_name(sol_pullback),
        playbook="sol_pullback_pair_dense_core",
    )
    add(
        sol_range,
        name="sol_range_ladder_long_adx21_cd2_lb17_zone021_s054",
        note="SOL stage193：range_ladder 继续只留一条辅助线。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 17,
            "strategy_params.breakout_atr_buffer": 0.48,
            "strategy_params.cooldown_bars": 2,
            "filters.adx_floor": 21,
        ...
        track="sol_range_ladder_dense",
        anchor_name=_name(sol_range),
        playbook="sol_range_ladder_dense",
    )
    add(
        sol_short,
        name="sol_guarded_short_accel_lb12_atr050_adx16_cd1_s078",
        note="SOL stage193：guarded short 只保一条不扩散。",
        family="short",
        patch={
        ...
        track="sol_guarded_short_dense",
        anchor_name=_name(sol_short),
        playbook="sol_guarded_short_dense",
    )
    return out
...
def _write_report(path_txt: Path, path_json: Path, main_rows: list[dict[str, Any]], branch_rows: list[dict[str, Any]], repaired_main: list[str], repaired_branch: list[str], scanned_main: list[str], scanned_branch: list[str], runtime_anchor: dict[str, Any], asset_summary: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    lines: list[str] = []
    lines.append("Stage193 anchor-plateau beta frontier")
    lines.append("")
    ...
    lines.append("")
    lines.append("[focus]")
    lines.append("- ETH: 围绕 reclaim_pair 新 active 做密集邻域，不再发散扫旧簇。")
    lines.append("- SOL: 围绕 pullback_pair 新 long_watch 做密集邻域，short 只留一条 guarded。")
    lines.append("- BTC: 只保 3 条中速参考线，不再占主算力。")
    lines.append("- 主线: 只保 1 条 BNB split-reentry 候选。")
    lines.append("")
    ...
    lines.append("- ETH 继续 reclaim_pair 主攻，先看 s064 周围的更快/更稳两档。")
    lines.append("- SOL 继续 pullback_pair 主攻，long_watch 先增厚样本；guarded short 只留辅助。")
    lines.append("- BTC 继续只保中速 squeeze / breakout / 1条 short。")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage193 anchor-plateau beta frontier")
    ...
    for item in _mainline_items(main_rows):
        print(f"[stage193] main {item['name']}", flush=True)
    ...
    for item in _branch_items(branch_rows):
        print(f"[stage193] branch {item['name']}", flush=True)
    ...
    frontier_txt = raw / "stage193_anchor_plateau_beta_frontier_latest.txt"
    frontier_json = raw / "stage193_anchor_plateau_beta_frontier_latest.json"
    ...
    manifest = {
        "mode": "anchor_plateau_beta_frontier",
        "target_monthly_min": TARGET_MONTHLY_MIN,
        "target_monthly_max": TARGET_MONTHLY_MAX,
        "repaired_main": repaired_main,
        "repaired_branch": repaired_branch,
    ...
    manifest_path = raw / "stage193_anchor_plateau_beta_frontier_manifest_latest.json"
    ...
    out_zip = root.parent / "Downloads" / "stage193_anchor_plateau_beta_frontier_latest.zip"
    WRITE_SINGLE_ZIP(out_zip, {
        "stage193_anchor_plateau_beta_frontier_latest.txt": frontier_txt,
        "stage193_anchor_plateau_beta_frontier_latest.json": frontier_json,
        "stage193_anchor_plateau_beta_frontier_manifest_latest.json": manifest_path,
