...
def _score(row: dict[str, Any], runtime_anchor: dict[str, Any], *, branch: bool) -> float:
    ...
    # Web-informed regime ranking:
    # - funding is a crowding/carry signal, not a standalone trigger
    # - rising OI confirms trend participation
    # - DVOL expansion favors trend/squeeze; stable vol favors range/grid
    if pb in {"eth_event_drift_web", "eth_event_drift_fast_web"}:
        score += 320.0
    if pb in {"eth_squeeze_follow_web", "eth_squeeze_follow_fast_web"}:
        score += 300.0
    if pb == "eth_retest_short_web":
        score += 150.0
    if pb == "eth_reclaim_beta_web":
        score += 40.0

    if pb == "btc_breakout_web":
        score += 170.0
    if pb == "btc_squeeze_follow_web":
        score += 130.0
    if pb == "btc_retest_short_web":
        score += 80.0

    if pb in {"sol_pullback_pair_web", "sol_range_ladder_web"}:
        score += 170.0
    if pb == "sol_guarded_short_web":
        score += 70.0
    ...
    # Demote reclaim-heavy ETH paths for now: keep as reserve, not main slot.
    if branch and sym == "eth" and "reclaim" in name:
        score -= 220.0

    # BTC: keep only breakout/squeeze/one short; hard reject known dead zones.
    if branch and sym == "btc" and ("break_fail" in name or "pair" in name and "event_pair" in name):
        score -= 99999.0

    # Trend/squeeze require enough trades and positive sample quality.
    if branch and sym == "eth" and pb in {"eth_event_drift_web", "eth_event_drift_fast_web", "eth_squeeze_follow_web", "eth_squeeze_follow_fast_web"}:
        if r_t >= 24 and w_t >= 18 and r_pf >= 2.2 and w_pf >= 2.2:
            score += 220.0
        if r_dd <= 0.055 and w_dd <= 0.055:
            score += 60.0
        if max(r_m, w_m) >= 0.012:
            score += 40.0

    # Grid/range only when drawdown stays contained and sample is not too thin.
    if branch and sym == "sol" and pb in {"sol_pullback_pair_web", "sol_range_ladder_web"}:
        if r_t >= 20 and w_t >= 15:
            score += 80.0
    ...
    if branch and sym == "sol" and "short" in name and (w_pf < 1.1 or w_t < 8):
        score -= 280.0

    if branch and sym == "btc":
        if pb == "btc_breakout_web" and r_t >= 15 and w_t >= 10 and r_pf >= 1.6 and w_pf >= 1.5:
            score += 110.0
        if pb == "btc_squeeze_follow_web" and r_t >= 18 and w_t >= 14 and r_pf >= 1.5 and w_pf >= 1.5:
            score += 80.0
        if r_t < 4 or w_t < 4:
            score -= 1800.0

    # Mainline: stop wasting rank on weak micro-variants.
    if not branch and pb != "base" and (w_m <= 0.0 or r_m <= 0.0):
        score -= 220.0
...
def _mainline_items() -> list[dict[str, Any]]:
    ...
    add(
        ...
        name="mainline_core_satellite_dynlev_fix8_lock18_bnbpulse_btc580_tp112_trail038",
        note="主线 stage189：只留一条 BNB pulse 结构验证，不再扫主线微邻域。",
        patch={
    ...
    add(
        ...
        name="mainline_split_adx24_cd4_lb20_zone024_bnbpulse_btc600_tp108_trail036",
        note="主线 stage189：只留一条 split pulse 备用。",
        patch={
...
def _branch_items() -> list[dict[str, Any]]:
    ...
    add(
        ...
        note="BTC stage189：trend 模式优先 breakout。",
        family="long",
        patch={
    ...
    add(
        ...
        track="btc_squeeze_follow_web",
        anchor_name="btc_breakout_long_event_lb20_atr060_adx24_s050",
        playbook="btc_squeeze_follow_web",
    )
    add(
        btc_short,
        name="btc_retest_short_event_lb16_atr052_adx20_s074",
        note="BTC stage189：只保一条 short 验证，不扩散。",
        family="short",
        patch={
    ...
    add(
        ...
        track="eth_event_drift_web",
        anchor_name=str(eth_anchor.get("name")) if eth_anchor else "",
        playbook="eth_event_drift_web",
    )
    add(
        eth_anchor,
        name="eth_event_drift_long_lb10_atr042_adx15_s048",
        note="ETH stage189：event drift 标准快版。",
        family="long",
        patch={
        ...
        track="eth_event_drift_fast_web",
        anchor_name=str(eth_anchor.get("name")) if eth_anchor else "",
        playbook="eth_event_drift_fast_web",
    )
    add(
        eth_anchor,
        name="eth_squeeze_follow_long_lb8_atr038_adx14_s052",
        note="ETH stage189：squeeze fast 版，和 drift 并行。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 8,
        ...
        track="eth_squeeze_follow_web",
        anchor_name=str(eth_anchor.get("name")) if eth_anchor else "",
        playbook="eth_squeeze_follow_web",
    )
    add(
        eth_anchor,
        name="eth_squeeze_follow_long_lb9_atr040_adx15_s048",
        note="ETH stage189：squeeze 标准快版。",
        family="long",
        patch={
    ...
    add(
        eth_short,
        name="eth_retest_short_trend_lb14_atr046_adx18_s074",
        note="ETH stage189：只保一条 short 反抽失败验证。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 14,
            "strategy_params.breakout_atr_buffer": 0.46,
        ...
        track="eth_retest_short_web",
        anchor_name=str(eth_short.get("name")) if eth_short else "",
        playbook="eth_retest_short_web",
    )
    add(
        eth_anchor,
        name="eth_reclaim_beta_long_lb10_atr040_adx14_cd1_s070",
        note="ETH stage189：reclaim 只留一条 beta 备用。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 10,
            "strategy_params.breakout_atr_buffer": 0.40,
    ...
    add(
        ...
        note="SOL stage189：range/grid 模式主攻 pair pullback。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 18,
    ...
    add(
        ...
        track="sol_range_ladder_web",
        anchor_name=str(sol_anchor.get("name")) if sol_anchor else "",
        playbook="sol_range_ladder_web",
    )
    add(
        sol_short,
        name="sol_guarded_short_accel_lb10_atr048_adx15_cd1_s080",
        note="SOL stage189：只留一条 guarded short。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 10,
        ...
        track="sol_guarded_short_web",
        anchor_name=str(sol_short.get("name")) if sol_short else "",
        playbook="sol_guarded_short_web",
    )
    return out
...
def _write_report(path_txt: Path, path_json: Path, main_rows: list[dict[str, Any]], branch_rows: list[dict[str, Any]], repaired_main: list[str], repaired_branch: list[str], scanned_main: list[str], scanned_branch: list[str], runtime_anchor: dict[str, Any]) -> dict[str, Any]:
    ...
    lines: list[str] = []
    lines.append("Stage189 web-blended regime frontier")
    lines.append("")
    ...
    lines.append("")
    lines.append("[regime_rules]")
    lines.append("- trend/squeeze: 只给 drift / breakout / squeeze 高权重；要求 trades/PF/DD 同时过线")
    lines.append("- range/grid: 只给 SOL pullback_pair / range_ladder 高权重；drawdown 和样本不过线直接降权")
    lines.append("- reclaim: 先保留，不再主攻")
    lines.append("- short: 每个币只留 1 条，避免算力分散")
    lines.append("")
    ...
    lines.append("- 这轮直接按 regime 分算力：ETH=drift/squeeze，BTC=breakout/squeeze，SOL=range/pullback。")
    lines.append("- 主线只留 1 条 BNB pulse + 1 条 split pulse 备用，减少低效率主线微调。")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage189 web-blended regime frontier")
    ...
    for item in _mainline_items():
        print(f"[stage189] main {item['name']}", flush=True)
    ...
    for item in _branch_items():
        print(f"[stage189] branch {item['name']}", flush=True)
    ...
    frontier_txt = raw / "stage189_regime_webblend_frontier_latest.txt"
    frontier_json = raw / "stage189_regime_webblend_frontier_latest.json"
    ...
    manifest_path = raw / "stage189_regime_webblend_frontier_manifest_latest.json"
    ...
    out_zip = root.parent / "Downloads" / "stage189_regime_webblend_frontier_latest.zip"
    WRITE_SINGLE_ZIP(out_zip, {
        "stage189_regime_webblend_frontier_latest.txt": frontier_txt,
        "stage189_regime_webblend_frontier_latest.json": frontier_json,
        "stage189_regime_webblend_frontier_manifest_latest.json": manifest_path,
