...
def _new_branch_items() -> list[dict[str, Any]]:
    ...
    anchor = "eth_reclaim_long_lb11_atr043_adx16_s060"
    variants = [
        (
            "eth_reclaim_long_lb11_atr043_adx16_s064",
            "ETH reclaim phase3：沿 s060 再上提一格预算，只看 plateau 是否仍稳。",
            {
        ...
        (
            ...
            "ETH reclaim phase3：激进档，只在近2年/WF 不塌时才允许排前。",
            {
                "strategy_params.breakout_lookback": 11,
                "strategy_params.breakout_atr_buffer": 0.43,
                "strategy_params.cooldown_bars": 4,
        ...
        (
            ...
            "ETH reclaim phase3：更高预算压力测试，专看月化能否继续抬升。",
            {
                "strategy_params.breakout_lookback": 11,
                "strategy_params.breakout_atr_buffer": 0.43,
                "strategy_params.cooldown_bars": 4,
        ...
        (
            "eth_reclaim_long_lb11_atr042_adx16_s060",
            "ETH reclaim phase3：更早接第二脚，用同档预算看提频是否能换来更高月化。",
            {
                "strategy_params.breakout_lookback": 11,
        ...
        (
            ...
            "ETH reclaim phase3：更保守确认，比较 s060 周边平台是否扩宽。",
            {
                "strategy_params.breakout_lookback": 11,
        ...
        (
            "eth_reclaim_hold_long_lb11_atr043_adx16_s052",
            "ETH reclaim+hold phase3：用中档预算验证 hold 版本是否只是低样本高 PF 假优。",
            {
                "strategy_params.breakout_lookback": 11,
                "strategy_params.breakout_atr_buffer": 0.43,
        ...
        (
            ...
            "ETH reclaim+hold phase3：更保守确认版，只看平台厚度，不追单点高 PF。",
            {
                "strategy_params.breakout_lookback": 11,
                "strategy_params.breakout_atr_buffer": 0.44,
                "strategy_params.cooldown_bars": 5,
        ...
        (
            "eth_reclaim_long_lb12_atr043_adx16_s060",
            "ETH reclaim phase3：慢半拍确认 + 中高预算，检验 plateau 是否更稳。",
            {
...
def _frontier_score(row: dict[str, Any], rows: list[dict[str, Any]], *, branch: bool) -> float:
    ...
    if sym == "eth" and fam == "long" and track == "reclaim":
        ...
        if month >= 0.025:
            bonus += 10.0
        if plateau_q >= 0.48 and neighbor_count >= 4:
            bonus += 20.0
        elif plateau_q >= 0.36 and neighbor_count >= 3:
            bonus += 10.0
        if abs(recent_monthly - wf_monthly) <= 0.008:
            bonus += 8.0

        # overfit / cliff guards
        if recent_trades < 24 or wf_trades < 16:
            penalty += 10.0
        if wf_monthly < 0.015 or wf_pf < 1.40:
            penalty += 18.0
        if wf_dd > 0.050:
            penalty += 12.0
        if (recent_pf > 6.0 or wf_pf > 10.0) and (recent_trades < 28 or wf_trades < 20):
            penalty += 22.0
...
def _write_report(
    ...
    lines: list[str] = []
    lines.append("Stage145 reclaim plateau phase3")
    lines.append("原则：继续只围绕 ETH reclaim 最强簇扩一圈；重点看 s060 周边能否抬月化，同时压住低样本高 PF 伪优。")
    ...
    if branch_payload:
        ...
        lines.append(f"- 分支第一名: {top['name']} | track={top['track']} | target_monthly={top['target_monthly']*100:.2f}% | plateau={top['plateau']:.2f} | gap_to_7.6={top['gap_to_floor']*100:.2f}%")
    if active_eth:
        lines.append(f"- stage91 ETH active: {active_eth}")
    lines.append("- 这轮只继续扩 ETH reclaim 邻域，并把 runtime/report 同步到最新 stage91；BTC/SOL 继续不占本轮算力。")

    payload = {
        "mode": "reclaim_plateau_phase3",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage145 reclaim plateau phase3")
    ...
    for item in _new_branch_items():
        print(f"[stage145] branch {item['name']}", flush=True)
    ...
    frontier_txt = raw / "stage145_reclaim_plateau_phase3_latest.txt"
    frontier_json = raw / "stage145_reclaim_plateau_phase3_latest.json"
    ...
    manifest_path = raw / "stage145_reclaim_plateau_phase3_manifest_latest.json"
