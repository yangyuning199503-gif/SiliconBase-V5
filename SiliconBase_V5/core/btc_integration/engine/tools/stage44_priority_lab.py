...
def _load_data(root: Path, cfg: dict[str, Any], symbols: list[str] | None = None, start: str = "", end: str = "") -> dict[str, pd.DataFrame]:
    data_cfg = copy.deepcopy(cfg.get("data", {}) or {})
    chosen = symbols or list(data_cfg.get("symbols", []))
...
def _score_main(row: dict[str, Any], base: dict[str, Any]) -> float:
    m = row.get("monthly", {}) or {}
    bm = base.get("monthly", {}) or {}
    trade_gain = max(0, int(row.get("trades", 0)) - int(base.get("trades", 0)))
    active_gain = max(0, int(m.get("active_months", 0)) - int(bm.get("active_months", 0)))
    ...
    return float(trade_gain * 1.15 + active_gain * 1.3 - pf_pen * 90.0 - dd_pen * 220.0 - ret_pen * 8.0 + floor_bonus)
...
def _score_branch(row: dict[str, Any]) -> float:
    ...
    return float(
        ...
        + m.get("months_ge_20", 0) * 4.0
        + m.get("monthly_p75", 0.0) * 18.0
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage44 priority lab")
    ...
    txt_out = Path(args.txt_out).expanduser().resolve() if args.txt_out else reports_raw / "stage44_priority_lab_latest.txt"
    json_out = Path(args.json_out).expanduser().resolve() if args.json_out else reports_raw / "stage44_priority_lab_latest.json"
    ...
    for row in main_rows:
        row["score"] = _score_main(row, base)
    ...
    for name, mods in SOL_VARIANTS:
        ...
        sol_rows.append(row)
    sol_sorted = sorted(sol_rows, key=lambda r: (r["score"], r["pf"], r["ret"]), reverse=True)
    best_sol = sol_sorted[0] if sol_sorted else None

    lines: list[str] = []
    lines.append("Stage44 优先实验（主线稳定性 + SOL shock）")
    ...
    for row in main_sorted:
        m = row.get("monthly", {}) or {}
        lines.append(
            f"- {row['name']}: trades={row['trades']} | pf={row['pf']:.3f} | ret={_pct(row['ret'])} | maxDD={_pct(row['maxdd'])} | active_months={m.get('active_months',0)} | months>=20%={m.get('months_ge_20',0)} | roll12_pf_floor={row.get('rolling12_pf_floor',0.0):.3f} | score={row['score']:+.2f}"
        )
        lines.append(
    ...
    for row in sol_sorted:
        m = row.get("monthly", {}) or {}
        lines.append(
            f"- {row['name']}: trades={row['trades']} | pf={row['pf']:.3f} | ret={_pct(row['ret'])} | maxDD={_pct(row['maxdd'])} | median_month={_pct(m.get('monthly_median',0.0))} | p75_month={_pct(m.get('monthly_p75',0.0))} | months>=20%={m.get('months_ge_20',0)} | roll12_pf_floor={row.get('rolling12_pf_floor',0.0):.3f} | score={row['score']:+.2f}"
        )
        lines.append(f"  counts={json.dumps(row.get('counts', {}), ensure_ascii=False)}")
    lines.append("")
    lines.append("=== 结论 ===")
    if best_main is not None:
        lines.append(f"- 主线先保留 {best_main['name']} 做第一候选；先看样本外稳定性，再决定是否并线。")
    lines.append("- 主线继续只做精准补漏，不做全局松绑。")
    if best_sol is not None:
        lines.append(f"- SOL 当前最优先方向是 {best_sol['name']}；若 month>=20% 命中仍差，就继续拆结构，不接 demo。")
    lines.append("- SOL short shock 只作为独立引擎研究，不和主线硬混。")
    lines.append("- Polymarket 继续只做 regime prior / risk gate，不直接做下单触发。")
    ...
    payload = {
        ...
        "note": {
            "user_target_branch_monthly": ">20%",
            "user_target_mainline": "higher trade frequency with controlled PF/DD",
