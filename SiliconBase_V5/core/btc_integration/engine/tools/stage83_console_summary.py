...
def main() -> None:
    ap = argparse.ArgumentParser(description="打印 stage81/82 中文摘要到终端")
    ...
    main_txt = _read_text(raw / "stage81_mainline_walkforward_latest.txt")
    branch_txt = _read_text(raw / "stage82_branch_walkforward_latest.txt")
    ...
    print("")
    _print_block("=== 主线WF 前2 ===", _bullet_groups(main_txt, "=== 候选结果 ===", 2))
    print("")
    _print_block("=== 分支WF 前4 ===", _bullet_groups(branch_txt, "=== 各赛道当前最优 ===", 4))
