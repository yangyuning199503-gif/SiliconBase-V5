...
ASSET_WEIGHTS = {"btc": 0.15, "eth": 0.75, "sol": 0.10}
ASSETS = ["btc", "eth", "sol"]
...
def _asset_candidate_weights(rows: list[dict[str, Any]]) -> dict[str, float]:
    counts: dict[str, int] = dict.fromkeys(ASSETS, 0)
    for row in rows:
    ...
    for row in rows:
        ...
        sym = str(row.get("symbol", "")).lower()
        base_w = float(ASSET_WEIGHTS.get(sym, 0.0))
        denom = max(counts.get(sym, 1), 1)
        weights[name] = base_w / float(denom)
    return weights
...
def _pick_rows(stage91: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in asset_summary:
        ...
        keys = ["active", "long_best", "short_best", "dual_best"]
        if sym in {"btc", "sol"}:
            keys = ["active", "short_best", "dual_best", "long_best"]
        for key in keys:
            row = item.get(key)
    ...
    for sym in ASSETS:
        ...
        limit = 1 if sym in {"btc", "sol"} else 2
        for r in extras[:limit]:
            name = str(r.get("name", ""))
...
def _branch_variants() -> list[dict[str, Any]]:
    ...
    return [
        {
            "name": "eth_core_balance_t016",
            "event_weight": 0.62,
            "deriv_weight": 0.38,
            "phase_weights": {"pre": 0.16, "release": 0.98, "drift": 0.72, "decay": 0.22},
            "boost_soft_thr": 0.12,
            "boost_mid_thr": 0.18,
            "cut_soft_thr": 0.16,
            "cut_mid_thr": 0.24,
            ...
            "net_cut_floor": -0.03,
            "boost_soft_asset_thresholds": {"btc": inf, "eth": 0.09, "sol": inf},
            "boost_mid_asset_thresholds": {"btc": inf, "eth": 0.14, "sol": inf},
            "cut_soft_asset_thresholds": {"btc": 0.14, "eth": 0.15, "sol": 0.14},
            "cut_mid_asset_thresholds": {"btc": 0.21, "eth": 0.21, "sol": 0.20},
            "allow_boost_assets": {"btc": False, "eth": True, "sol": False},
            "allow_cut_assets": {"btc": True, "eth": True, "sol": True},
        },
        {
            "name": "eth_core_capture_t012",
            "event_weight": 0.66,
            "deriv_weight": 0.34,
            "phase_weights": {"pre": 0.14, "release": 1.00, "drift": 0.78, "decay": 0.22},
            "boost_soft_thr": 0.10,
            "boost_mid_thr": 0.16,
            "cut_soft_thr": 0.17,
            "cut_mid_thr": 0.25,
            ...
            "net_cut_floor": -0.03,
            "boost_soft_asset_thresholds": {"btc": inf, "eth": 0.07, "sol": inf},
            "boost_mid_asset_thresholds": {"btc": inf, "eth": 0.12, "sol": inf},
            "cut_soft_asset_thresholds": {"btc": 0.15, "eth": 0.16, "sol": 0.15},
            "cut_mid_asset_thresholds": {"btc": 0.22, "eth": 0.22, "sol": 0.21},
            "allow_boost_assets": {"btc": False, "eth": True, "sol": False},
            "allow_cut_assets": {"btc": True, "eth": True, "sol": True},
        },
        {
            "name": "multi_asset_cutonly_t020",
            "event_weight": 0.60,
            "deriv_weight": 0.40,
            "phase_weights": {"pre": 0.18, "release": 0.96, "drift": 0.64, "decay": 0.20},
            "boost_soft_thr": inf,
            "boost_mid_thr": inf,
            "cut_soft_thr": 0.15,
            "cut_mid_thr": 0.22,
            "boost_soft_scale": 1.0,
            "boost_mid_scale": 1.0,
            "cut_soft_scale": 0.84,
            "cut_mid_scale": 0.60,
            "net_boost_floor": 1.0,
            "net_cut_floor": -0.02,
            "boost_soft_asset_thresholds": {"btc": inf, "eth": inf, "sol": inf},
            "boost_mid_asset_thresholds": {"btc": inf, "eth": inf, "sol": inf},
            "cut_soft_asset_thresholds": {"btc": 0.14, "eth": 0.15, "sol": 0.14},
            "cut_mid_asset_thresholds": {"btc": 0.20, "eth": 0.20, "sol": 0.19},
            "allow_boost_assets": {"btc": False, "eth": False, "sol": False},
            "allow_cut_assets": {"btc": True, "eth": True, "sol": True},
        },
        {
            "name": "eth_core_preserve_t022",
            "event_weight": 0.58,
            "deriv_weight": 0.42,
            "phase_weights": {"pre": 0.14, "release": 0.90, "drift": 0.54, "decay": 0.16},
            "boost_soft_thr": 0.14,
            "boost_mid_thr": 0.22,
            "cut_soft_thr": 0.18,
            "cut_mid_thr": 0.27,
            ...
            "net_cut_floor": -0.04,
            "boost_soft_asset_thresholds": {"btc": inf, "eth": 0.11, "sol": inf},
            "boost_mid_asset_thresholds": {"btc": inf, "eth": 0.17, "sol": inf},
            "cut_soft_asset_thresholds": {"btc": 0.16, "eth": 0.17, "sol": 0.16},
            "cut_mid_asset_thresholds": {"btc": 0.24, "eth": 0.24, "sol": 0.23},
            "allow_boost_assets": {"btc": False, "eth": True, "sol": False},
            "allow_cut_assets": {"btc": True, "eth": True, "sol": True},
        },
    ]
...
def _candidate_eval(root: Path, row: dict[str, Any], hist: dict[str, Any], initial_equity: float) -> dict[str, Any]:
    ...
    for variant in _branch_variants():
        ...
        composite = float(
            0.24 * full_ev["score"]
            + 0.98 * recent_ev["score"]
            + 0.94 * wf_score
            + 0.14 * gm_r2["monthlyized"]
            + 0.03 * recent_pf_c
            + 0.02 * full_pf_c
            - sign_penalty
        )
        scored.append({
            ...
            "sample_penalty": sample_penalty + sign_penalty,
...
def run(project_dir: Path, out_txt: Path, out_json: Path) -> None:
    ...
    candidate_weight_map = _asset_candidate_weights(selected)
    aggregate_rows: dict[str, dict[str, Any]] = {}
    eth_core_names = {
        str(r["candidate"].get("name", ""))
        for r in candidate_results
        if str(r["candidate"].get("symbol", "")).lower() == "eth"
    }
    for cr in candidate_results:
        ...
        for vr in cr.get("variants", []):
            bucket = aggregate_rows.setdefault(vr["name"], {
                ...
                "eth_core_recent_ret": 0.0,
                "eth_core_recent_pf": 0.0,
                "eth_core_wf_delta": 0.0,
                "members": [],
            })
            ...
            if row_name in eth_core_names:
                bucket["eth_core_recent_ret"] += candidate_weight * float(gm_r2.get("total_return", 0.0))
                bucket["eth_core_recent_pf"] += candidate_weight * float(vr.get("recent_pf_capped", 0.0))
                bucket["eth_core_wf_delta"] += candidate_weight * float(wf.get("aggregate_pnl_delta", 0.0))
    ...
    variant_leaderboard = sorted(
        ...
        key=lambda x: (x["weighted_score"], x["eth_core_recent_ret"], x["weighted_recent_ret"], -x["weighted_negative_recent"]),
        reverse=True,
    )
    ...
    for v in variant_leaderboard:
        if (
            ...
            and float(v.get("weighted_recent_pf", 0.0)) >= 1.08
            and float(v.get("weighted_wf_pnl_delta", 0.0)) >= 0.0
            and float(v.get("weighted_negative_recent", 0.0)) <= 0.30
            and float(v.get("eth_core_recent_ret", 0.0)) > 0
        ):
    ...
    lines: list[str] = []
    lines.append("Stage216 branch ETH-core overlay guard frontier")
    lines.append("")
    lines.append("[model]")
    lines.append("- 分支继续：技术面决定 entry；消息面/事件面只做 sizing overlay。")
    lines.append("- 这版把 overlay 压缩到 ETH 主簇；BTC/SOL 只允许保守 CUT，不做激进 BOOST。")
    lines.append("- 目标：减少 BTC/SOL 负拖累，同时保留 ETH 主簇的正向增益。")
    lines.append("")
    ...
    for v in variant_leaderboard[:4]:
        lines.append(
            f"- {v['name']} | weighted_score={v['weighted_score']:+.4f} | weighted_2y_month={_fmt_pct(v['weighted_recent_monthly'])} weighted_2y_ret={_fmt_pct(v['weighted_recent_ret'])} weighted_2y_pf_cap={v['weighted_recent_pf']:.3f} | eth_core_2y_ret={_fmt_pct(v['eth_core_recent_ret'])} eth_core_wf_delta={_fmt_num(v['eth_core_wf_delta'])} | weighted_wf_pnl_delta={_fmt_num(v['weighted_wf_pnl_delta'])} | neg_ratio={_fmt_pct(v['weighted_negative_recent'])}"
        )
    lines.append("")
    ...
    if recommended_variant:
        lines.append(f"- 推荐先用 {recommended_variant['name']} 做 ETH-core sizing overlay 基线。")
        lines.append("- 原因：ETH 主簇保留增益，BTC/SOL 只做保守 CUT 后，全局拖累更小。")
        lines.append("- 下一步只做一件事：把推荐 variant 作为 branch runtime 的仓位覆盖预览，不改 entry。")
    else:
        lines.append("- 这轮仍不建议把 overlay 直接同步到 branch runtime。")
        lines.append("- 原因：即使压成 ETH 主簇 + BTC/SOL cut-only，全局加总后仍未形成稳定正增益。")
        lines.append("- 下一步只做一件事：继续把 overlay 收缩到 ETH reclaim 主簇，并把 BTC/SOL 降成系统性减仓开关。")
