...
def infer_family(name: str, row: dict[str, Any], is_branch: bool) -> str:
    if is_branch:
        ...
        return "_".join(tokens[:3]) if len(tokens) >= 3 else name.lower()
    lower = name.lower()
    for prefix in [
...
def get_recent_metrics(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(dom, dict):
        ...
        if isinstance(metrics, dict):
            # Fallback only if it already looks like a recent-window metric
...
def metric_summary(metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "months_ge_10": _safe_int(monthly.get("months_ge_10")),
    }
...
def diversity_cap(audits: list[RowAudit], *, is_branch: bool) -> dict[str, list[dict[str, Any]]]:
    sorted_rows = sorted(audits, key=lambda x: x.alpha_score, reverse=True)
    if not is_branch:
        fam_cap = 2
        picked: list[RowAudit] = []
        fam_counts: Counter = Counter()
        for row in sorted_rows:
            if row.dead_recent_wf_zero or row.full_zero:
                continue
    ...
    by_symbol: dict[str, list[RowAudit]] = defaultdict(list)
    for row in sorted_rows:
...
def format_report(summary: dict[str, Any], main_audits: list[RowAudit], branch_audits: list[RowAudit], curated: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("Stage181 搜索全面性 / 死角 / family diversity guard")
    lines.append("")
    lines.append("[main]")
    lines.append(f"- rows={summary['main']['rows']} dead_recent_wf_zero={summary['main']['dead_recent_wf_zero']} full_zero={summary['main']['full_zero']}")
    lines.append(f"- dominant_family={summary['main']['dominant_family']} dominant_ratio={summary['main']['dominant_ratio']}")
    lines.append(f"- top8_family_counts={json.dumps(summary['main']['top8_family_counts'], ensure_ascii=False)}")
    lines.append("")
    lines.append("[branch]")
    lines.append(f"- rows={summary['branch']['rows']} dead_recent_wf_zero={summary['branch']['dead_recent_wf_zero']} full_zero={summary['branch']['full_zero']}")
    lines.append(f"- dominant_family={summary['branch']['dominant_family']} dominant_ratio={summary['branch']['dominant_ratio']}")
    lines.append(f"- top12_family_counts={json.dumps(summary['branch']['top12_family_counts'], ensure_ascii=False)}")
    lines.append(f"- symbol_counts={json.dumps(summary['branch']['symbol_counts'], ensure_ascii=False)}")
    lines.append("")
    lines.append("[guard]")
    lines.append(f"- missing_guard_fields={summary['guard_gaps']}")
    lines.append(f"- low_sample_high_pf={summary['low_sample_high_pf']}")
    lines.append("")
    lines.append("[curated_main]")
    for row in curated.get("main", []):
        lines.append(
    ...
    for symbol in sorted(k for k in curated if k != 'main'):
        lines.append("")
        lines.append(f"[curated_{symbol}]")
        for row in curated[symbol]:
            lines.append(
                f"- {row['name']} | family={row['family']} | recent_monthly={percent(row['recent_monthlyized'])} | wf_monthly={percent(row['wf_monthlyized'])} | recent_trades={row['recent_trades']} | wf_trades={row['wf_trades']}"
            )
    lines.append("")
    ...
    if summary["main"]["dominant_ratio"] > 0.5 or summary["branch"]["dominant_ratio"] > 0.5:
        lines.append("- 先加 family diversity cap，再跑新 frontier；不要继续让单一家族占满前排。")
    if summary["main"]["dead_recent_wf_zero"] or summary["branch"]["dead_recent_wf_zero"]:
        lines.append("- 先把 dead_recent_wf_zero 模板黑名单化，再继续创新。")
    if summary["guard_gaps"]:
        lines.append("- 先补齐 dominant_gate / walkforward 缺字段，再继续排序。")
    if not any([summary["main"]["dominant_ratio"] > 0.5, summary["branch"]["dominant_ratio"] > 0.5, summary["guard_gaps"]]):
        lines.append("- 搜索面已基本过关；可以继续做激进 frontier。")
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    stage90_path = research_dir / "stage90_mainline_event_alpha_matrix_latest.json"
    stage91_path = research_dir / "stage91_branch_event_alpha_matrix_latest.json"

    stage90 = load_json(stage90_path)
    stage91 = load_json(stage91_path)
    ...
    curated = diversity_cap(main_audits, is_branch=False)
    curated.update(diversity_cap(branch_audits, is_branch=True))

    txt_path = downloads_dir / "stage181_frontier_coverage_diversity_guard_latest.txt"
    json_path = downloads_dir / "stage181_frontier_coverage_diversity_guard_latest.json"
    main_dead_path = downloads_dir / "stage181_main_dead_templates_latest.json"
    branch_dead_path = downloads_dir / "stage181_branch_dead_templates_latest.json"
    curated_path = downloads_dir / "stage181_curated_shortlist_latest.json"

    txt_path.write_text(format_report(summary, main_audits, branch_audits, curated), encoding="utf-8")
    write_json(json_path, {
        "summary": summary,
        "curated": curated,
        "main_audits": [asdict(x) for x in main_audits],
        "branch_audits": [asdict(x) for x in branch_audits],
    })
    write_json(main_dead_path, {
    ...
    bundle = downloads_dir / "stage181_frontier_coverage_diversity_guard_latest.zip"
    with zipfile.ZipFile(bundle, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for p in [txt_path, json_path, main_dead_path, branch_dead_path, curated_path]:
