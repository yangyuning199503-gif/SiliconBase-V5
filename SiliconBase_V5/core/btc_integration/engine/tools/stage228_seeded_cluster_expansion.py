...
class SeededClusterExpansion:
    ...
    def _build_pairings(self) -> list[tuple[str, str]]:
        ...
        for sym in self.symbols:
            for pair in SEED_PAIRINGS.get(sym, []):
                if pair not in all_pairs:
                    all_pairs.append(pair)
        return all_pairs

    def pairings_for_symbol(self, symbol: str) -> list[tuple[str, str]]:
        pairs: list[tuple[str, str]] = []
        for entry_tf, filter_tf in SEED_PAIRINGS.get(symbol, self.pairings):
    ...
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        out["bb_width_q30"] = out["bb_width"].rolling(80, min_periods=20).quantile(0.30)
        out["bb_width_q70"] = out["bb_width"].rolling(80, min_periods=20).quantile(0.70)

        rng = (out["high"] - out["low"]).replace(0, np.nan)
        upper = out["high"] - out[["open", "close"]].max(axis=1)
        lower = out[["open", "close"]].min(axis=1) - out["low"]
        out["upper_wick_ratio"] = (upper / rng).clip(lower=0)
        out["lower_wick_ratio"] = (lower / rng).clip(lower=0)
        out["wick_max_ratio"] = pd.concat([out["upper_wick_ratio"], out["lower_wick_ratio"]], axis=1).max(axis=1)
    ...
    def signal_family(df: pd.DataFrame, family: str, p: dict[str, float]) -> tuple[np.ndarray, np.ndarray]:
        ...
        sweep_long = (
            (low < df["ll10"].shift(1))
            & (c > df["ll10"].shift(1))
            & (df["lower_wick_ratio"] > p["sweep_wick_min"])
            & (df["range_atr"] < p["wick_atr"] * 1.4)
        )
        sweep_short = (
            (h > df["hh10"].shift(1))
            & (c < df["hh10"].shift(1))
            & (df["upper_wick_ratio"] > p["sweep_wick_min"])
            & (df["range_atr"] < p["wick_atr"] * 1.4)
        )

        fake_break_long = (
            (low.shift(1) < df["ll20"].shift(2) - p["bo_atr"] * atr_.shift(1))
            & (c > df["ll20"].shift(1))
            & (c > o)
        )
        fake_break_short = (
            (h.shift(1) > df["hh20"].shift(2) + p["bo_atr"] * atr_.shift(1))
            & (c < df["hh20"].shift(1))
            & (c < o)
        )

        squeeze_on = bbw.shift(1) < df["bb_width_q30"].shift(1)
        squeeze_release = bbw > bbw.shift(1)
        pullback_long = (low <= ef) & (c > ef) & (c > bbm)
        pullback_short = (h >= ef) & (c < ef) & (c < bbm)

        range_env = (fadx < p["range_adx_cap"]) & (frsi.between(42, 58))

        if family == "bb_meanrev":
            long = (c < bbd) & (rsi_ < 34) & (frsi > 48) & wick_ok
            short = (c > bbu) & (rsi_ > 66) & (frsi < 52) & wick_ok
        elif family == "range_grid_bb":
            long = range_env & (c < bbd) & (rsi_ < 38) & (df["range_atr"] < p["wick_atr"])
            short = range_env & (c > bbu) & (rsi_ > 62) & (df["range_atr"] < p["wick_atr"])
        elif family == "liquidity_sweep_reclaim":
            long = sweep_long & (frsi > 46) & (c > ef) & (c > vwap)
            short = sweep_short & (frsi < 54) & (c < ef) & (c < vwap)
        elif family == "retest_fail":
            long = fake_break_long & (frsi > 46) & (c > ef)
            short = fake_break_short & (frsi < 54) & (c < ef)
        elif family == "squeeze_pullback":
            long = squeeze_on & squeeze_release & trend_up & trend_strong & pullback_long & (macdh > 0) & wick_ok
            short = squeeze_on & squeeze_release & trend_dn & trend_strong & pullback_short & (macdh < 0) & wick_ok
        else:
    ...
    @staticmethod
    def soft_guard(r: pd.Series) -> str:
        if (r["full_dd"] <= -80) or (r["full_ret"] <= -80):
            return "research_only_long_tail_fail"
        if (r["full_dd"] <= -60) or (r["full_pf"] < 0.90):
            return "research_only_soft_fail"
        return "promote_research"

    def run(self) -> dict[str, object]:
        rows: list[dict[str, float]] = []
        skipped: list[dict[str, str]] = []
        for symbol in self.symbols:
            for entry_tf, filter_tf in SEED_PAIRINGS.get(symbol, []):
                if entry_tf == "5m" and not (self.raw_dir / f"{symbol}_5m.csv").exists():
                    skipped.append({"symbol": symbol, "entry_tf": entry_tf, "filter_tf": filter_tf, "reason": "missing_5m_raw"})
        for symbol in self.symbols:
            print(f"[stage228] symbol={symbol}", flush=True)
            for entry_tf, filter_tf in self.pairings_for_symbol(symbol):
                print(f"[stage228] pairing={symbol}:{entry_tf}/{filter_tf}", flush=True)
                for family in FAMILIES:
                    for p in PARAMS:
                        rows.append(self.evaluate(symbol, entry_tf, filter_tf, family, p))
                if rows:
                    pd.DataFrame(rows).to_csv(self.out_dir / "stage228_seeded_cluster_expansion_all.partial.csv", index=False)
        df = pd.DataFrame(rows)
        df["hard_pass"] = df.apply(self.hard_filter, axis=1)
        df["promotion_lane"] = df.apply(self.soft_guard, axis=1)
        df.to_csv(self.out_dir / "stage228_seeded_cluster_expansion_all.csv", index=False)
        ...
        for symbol in self.symbols:
            ...
            summary_top[symbol] = pool.head(8).to_dict(orient="records")

        family_counts = (
            passed.groupby(["family"], as_index=False)
            .size()
            .rename(columns={"size": "hard_pass_count"})
            .sort_values(["hard_pass_count", "family"], ascending=[False, True])
        )
        combo_counts = (
        ...
        lane_counts = (
            passed.groupby(["promotion_lane"], as_index=False)
            .size()
            .rename(columns={"size": "count"})
            .sort_values(["count", "promotion_lane"], ascending=[False, True])
        )

        summary = {
            "status": "OK",
            "stage": "stage228_seeded_cluster_expansion",
            "tested_symbols": self.symbols,
            "pairings_used": self.pairings,
            "pairings_used_by_symbol": {sym: self.pairings_for_symbol(sym) for sym in self.symbols},
            ...
            "ranking_policy": {
                ...
                "full_sample": "soft_constraint_only_with_soft_guard",
            ...
            "soft_guard": {
                "promote_research": "full_dd>-60 and full_pf>=0.90 and full_ret>-80",
                "research_only_soft_fail": "full_dd<=-60 or full_pf<0.90",
                "research_only_long_tail_fail": "full_dd<=-80 or full_ret<=-80",
            },
            "families": FAMILIES,
            "params": PARAMS,
            "top_by_symbol": summary_top,
            "family_breadth": family_counts.to_dict(orient="records"),
            "shared_transferability": combo_counts.head(20).to_dict(orient="records"),
            "promotion_lanes": lane_counts.to_dict(orient="records"),
        }
        (self.out_dir / "stage228_seeded_cluster_expansion_summary.json").write_text(
            json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8"
        )

        lines: list[str] = []
        lines.append("[stage228_seeded_cluster_expansion]")
        ...
        lines.append("primary=近2年优先 | wf=近12个月样本外 | 6年仅做软约束，但加 soft guard")
        lines.append("")
        for symbol, items in summary_top.items():
            lines.append(f"[{symbol}]")
            for r in items:
                lines.append(
                    "- {family} | {entry_tf}/{filter_tf} | {param_id} | lane={promotion_lane} | "
                    "recent_win={recent_win:.2f}% | recent_ret={recent_ret:.2f}% | recent_pf={recent_pf:.3f} | "
                    "recent_dd={recent_dd:.2f}% | wf_ret={wf_ret:.2f}% | wf_pf={wf_pf:.3f} | full_ret={full_ret:.2f}% | full_dd={full_dd:.2f}%".format(**r)
                )
            lines.append("")
        lines.append("[family_breadth]")
        for r in summary["family_breadth"][:10]:
            lines.append(f"- {r['family']} | hard_pass_count={r['hard_pass_count']}")
        lines.append("")
        lines.append("[shared_transferability]")
        for r in summary["shared_transferability"][:12]:
            lines.append(
                f"- {r['family']} | {r['entry_tf']}/{r['filter_tf']} | {r['param_id']} | symbols_hit={r['symbols_hit']}"
            )
        lines.append("")
        lines.append("[promotion_lanes]")
        for r in summary["promotion_lanes"]:
            lines.append(f"- {r['promotion_lane']} | count={r['count']}")
        (self.out_dir / "stage228_seeded_cluster_expansion_latest.txt").write_text("\n".join(lines), encoding="utf-8")
        return summary
...
def main() -> None:
    ...
    args = ap.parse_args()
    lab = SeededClusterExpansion(Path(args.project_dir), [s.strip() for s in args.symbols.split(",") if s.strip()])
    lab.run()
