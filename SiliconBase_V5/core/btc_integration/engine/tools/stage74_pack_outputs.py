...
def _build_deepseek(okx_txt: str, msg_txt: str, main_txt: str, branch_txt: str, local_txt: str) -> str:
    ...
    lines.append("")
    lines.append("三、主线桥接前沿")
    lines.extend(_top_lines(main_txt, 14) or ["- stage73_mainline_bridge_frontier_latest.txt 缺失"])
    lines.append("")
    lines.append("四、分支重建赛道")
    lines.extend(_top_lines(branch_txt, 18) or ["- stage74_branch_rebuild_latest.txt 缺失"])
    lines.append("")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage74 pack outputs")
    ...
    main_txt_p = reports_raw / "stage73_mainline_bridge_frontier_latest.txt"
    main_json_p = reports_raw / "stage73_mainline_bridge_frontier_latest.json"
    branch_txt_p = reports_raw / "stage74_branch_rebuild_latest.txt"
    branch_json_p = reports_raw / "stage74_branch_rebuild_latest.json"
    ...
    if stale:
        raise SystemExit(f"stage73/74 报告没有被本轮刷新：{', '.join(stale)}")
    ...
    with zipfile.ZipFile(bundle_out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        readme = [
            ...
            f"project_dir={root}",
            f"stage73_mainline_mtime_utc={datetime.fromtimestamp(main_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            f"stage74_branch_mtime_utc={datetime.fromtimestamp(branch_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            "",
            "files:",
        ]
    ...
    must_have = {"README_SEND_TO_CHATGPT.txt", "stage73_mainline_bridge_frontier_latest.txt", "stage74_branch_rebuild_latest.txt"}
