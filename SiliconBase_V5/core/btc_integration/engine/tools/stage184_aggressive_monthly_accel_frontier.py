...
def _aggressive_score(row: dict[str, Any], *, branch: bool) -> float:
    ...
    score = 52000.0 * r_m + 47000.0 * w_m + 14.0 * r_t + 12.0 * w_t + 2.0 * full_t
    score += 90.0 * min(r_pf, 6.0) + 80.0 * min(w_pf, 6.0)
    score -= 110.0 * (r_dd * 100.0) + 90.0 * (w_dd * 100.0)

    pb = _playbook(row)
    bonus = 0.0
    if pb in {"eth_event_drift", "eth_squeeze_follow"}:
        bonus += 220.0
    if pb in {"main_ladder_pyramid", "main_banded_reentry"}:
        bonus += 140.0
    if pb == "main_overlay_hold":
        bonus += 60.0
    if pb in {"btc_breakout_long", "btc_event_drift", "btc_squeeze_follow", "btc_dual_hedge"}:
        bonus += 80.0
    if pb in {"sol_range_ladder", "sol_pullback_grid", "sol_pullback_pair"}:
        bonus += 60.0
    ...
    if branch and _symbol(row, branch=True) == "sol" and "short" in name.lower():
        penalty += 80.0
    if branch and _symbol(row, branch=True) == "btc" and "break_fail" in name.lower():
        penalty += 200.0
...
def _status(row: dict[str, Any], *, branch: bool) -> str:
    ...
    if _hard_blacklisted(str(row.get("name") or "")):
        return "blacklist"
    if r_m >= 0.03 and w_m >= 0.025 and r_pf >= 1.6 and w_pf >= 1.4 and r_t >= 10 and w_t >= 8:
        return "demo_ready"
    if r_m >= 0.02 and w_m >= 0.015 and r_pf >= 1.4 and w_pf >= 1.2 and r_t >= 8 and w_t >= 6:
        return "accel_track"
    if r_m > 0 and w_m > 0:
        return "reserve+"
    if r_m > 0 or w_m > 0:
...
def _mainline_items() -> list[dict[str, Any]]:
    ...
    add(
        "mainline_live_dynlev_fix8_lock18_ladder_pyramid_lb20_buf044_cd4_add6_step12",
        "主线 stage184：更激进的 ladder pyramiding，只在 fix8_lock18 母体上提频。",
        {
            ...
            "strategy_params.pyramiding_max_adds": 6,
            "strategy_params.add_step_atr": 1.2,
            "strategy_params.add_risk_fraction": 0.18,
            "strategy_params.breakeven_atr": 1.3,
            "money_management.stake_scale.bnb_long": 1.60,
    ...
    add(
        "mainline_live_dynlev_fix8_lock18_banded_reentry_lb20_buf042_cd3_add4_step14",
        "主线 stage184：banded reentry，目标是提高交易密度，不改 runtime anchor。",
        {
            "strategy_params.breakout_lookback": 20,
            ...
            "strategy_params.add_risk_fraction": 0.18,
            "strategy_params.breakeven_atr": 1.5,
    ...
    add(
        "mainline_live_dynlev_fix8_lock18_overlay_hold_lb24_buf046_cd8_gate_mix",
        "主线 stage184：overlay hold + event mix，保留质量同时小幅提频。",
        {
            ...
            "money_management.stake_scale.bnb_long": 1.36,
...
def _branch_items() -> list[dict[str, Any]]:
    ...
    eth_shock = item_map.get("eth_short_shock_guarded_lb20_atr062_adx28_s070") or item_map.get("eth_short_shock_fast_lb16_atr052_adx22_s078")
    ...
    add(
        ...
        note="BTC stage184：保留 event breakout，但略微提频。",
        family="long",
        patch={
    ...
    add(
        btc_long,
        name="btc_event_drift_long_lb14_atr048_adx18_s060",
        note="BTC stage184：事件后 drift，不追第一根。",
        family="long",
        patch={
    ...
    add(
        ...
        note="BTC stage184：只保留 retest short，不碰 break_fail。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 18,
            "strategy_params.breakout_atr_buffer": 0.56,
    ...
    add(
        ...
        note="BTC stage184：双向腿只保留 hedge band，不拆第三终端。",
        family="dual",
        patch={
            "strategy_params.cooldown_bars": 1,
    ...
    add(
        ...
        note="ETH stage184：事件漂移长腿，优先冲交易密度。",
        family="long",
        patch={
    ...
    add(
        ...
        note="ETH stage184：squeeze follow 与 drift 并行，不再只围 reclaim。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 10,
            "strategy_params.breakout_atr_buffer": 0.42,
    ...
    add(
        ...
        note="ETH stage184：reclaim 不删除，但降为一条保留赛道。",
        family="long",
        patch={
    ...
    add(
        eth_short,
        name="eth_retest_short_trend_lb20_atr060_adx24_cd3_s068",
        note="ETH stage184：保留 retest short，防止空腿断代。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 20,
    ...
    add(
        ...
        note="ETH stage184：保留一条 shock short，作为事件失败腿。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 20,
        ...
        anchor_name=str(eth_shock.get("name")) if eth_shock else "",
    ...
    add(
        ...
        note="SOL stage184：优先 long family，不再抢做负收益 short。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 20,
            "strategy_params.breakout_atr_buffer": 0.48,
    ...
    add(
        ...
        note="SOL stage184：grid-like pullback，只在 long family 里测。",
        family="long",
        patch={
            "strategy_params.breakout_lookback": 20,
    ...
    add(
        ...
        note="SOL stage184：保留一条 pair/pullback 长腿。",
        family="long",
        patch={
    ...
    add(
        ...
        note="SOL stage184：空腿只留一条 guarded short，不再占主算力。",
        family="short",
        patch={
            "strategy_params.breakout_lookback": 14,
...
def _write_report(path_txt: Path, path_json: Path, main_rows: list[dict[str, Any]], branch_rows: list[dict[str, Any]], repaired_main: list[str], repaired_branch: list[str], scanned_main: list[str], scanned_branch: list[str], active_map: dict[str, str]) -> dict[str, Any]:
    ...
    lines: list[str] = []
    lines.append("Stage184 aggressive monthly accel frontier")
    lines.append("")
    ...
    lines.append("- 这版不动 demo / 下单链路；只做研究层 aggressive frontier。")
    lines.append("- BTC break_fail 继续黑名单；ETH 由 reclaim 单核改成 drift+squeeze 主攻；SOL 继续 long 优先。")
    txt = "\n".join(lines).rstrip() + "\n"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage184 aggressive monthly accel frontier")
    ...
    for item in _mainline_items():
        print(f"[stage184] main {item['name']}", flush=True)
    ...
    for item in _branch_items():
        print(f"[stage184] branch {item['name']}", flush=True)
    ...
    frontier_txt = raw / "stage184_aggressive_monthly_accel_frontier_latest.txt"
    frontier_json = raw / "stage184_aggressive_monthly_accel_frontier_latest.json"
    ...
    manifest = {
        "mode": "aggressive_monthly_accel_frontier",
        "target_monthly_min": TARGET_MONTHLY_MIN,
        "target_monthly_max": TARGET_MONTHLY_MAX,
        "repaired_main": repaired_main,
        "repaired_branch": repaired_branch,
    ...
    manifest_path = raw / "stage184_aggressive_monthly_accel_manifest_latest.json"
    ...
    out_zip = root.parent / "Downloads" / "stage184_aggressive_monthly_accel_frontier_latest.zip"
    _write_single_zip(out_zip, {
        "stage184_aggressive_monthly_accel_frontier_latest.txt": frontier_txt,
        "stage184_aggressive_monthly_accel_frontier_latest.json": frontier_json,
        "stage184_aggressive_monthly_accel_manifest_latest.json": manifest_path,
