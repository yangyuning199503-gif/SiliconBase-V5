...
def _build_deepseek(okx_txt: str, main_txt: str, branch_txt: str) -> str:
    ...
    lines.append("")
    lines.append("二、Stage59 主线结构型提频")
    lines.extend(_top_lines(main_txt, 18) or ["- stage59_mainline_structural_latest.txt 缺失"])
    lines.append("")
    lines.append("三、Stage59 ETH/SOL 广角结构")
    lines.extend(_top_lines(branch_txt, 22) or ["- stage59_branch_structural_latest.txt 缺失"])
    lines.append("")
    lines.append("四、请重点给建议")
    lines.append("- 主线里 neutral_wick 与 neutral_wick_or_spike 哪个更适合并线？")
    lines.append("- ETH / SOL 是否适合把 neutral-news wick reversion 做成独立子引擎？")
    lines.append("- 哪个分支更值得先追求月化提升：SOL 双边，还是 ETH 双边？")
    return "\n".join(lines).rstrip() + "\n"


def main() -> None:
    ap = argparse.ArgumentParser(description="Stage59 pack outputs")
    ...
    main_txt_p = reports_raw / "stage59_mainline_structural_latest.txt"
    main_json_p = reports_raw / "stage59_mainline_structural_latest.json"
    branch_txt_p = reports_raw / "stage59_branch_structural_latest.txt"
    branch_json_p = reports_raw / "stage59_branch_structural_latest.json"
    ...
    if stale:
        raise SystemExit(f"stage59 报告没有被本轮刷新：{', '.join(stale)}")
    ...
    with zipfile.ZipFile(bundle_out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        readme = [
            ...
            f"project_dir={root}",
            f"stage59_mainline_mtime_utc={datetime.fromtimestamp(main_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            f"stage59_branch_mtime_utc={datetime.fromtimestamp(branch_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            "",
            "包含文件：",
        ]
    ...
    must_have = {"README_SEND_TO_CHATGPT.txt", "stage59_mainline_structural_latest.txt", "stage59_branch_structural_latest.txt"}
