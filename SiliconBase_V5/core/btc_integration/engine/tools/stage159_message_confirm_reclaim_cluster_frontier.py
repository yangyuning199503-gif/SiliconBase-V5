...
def _new_mainline_items() -> list[dict[str, Any]]:
    ...
    variants = [
        (
            "mainline_live_dynlev_fix8_lock18_lb26_buf046_cd16",
            "主线 phase159：更温和的提频邻域，验证消息确认能否帮助提频而不塌 PF。",
            {
                ...
                "money_management.stake_scale.bnb_long": 1.06,
        ...
        (
            "mainline_live_dynlev_fix8_lock18_lb24_buf045_cd14",
            "主线 phase159：更浅 buffer + 适中 cooldown，尽量提频但不碰极端激进区。",
            {
        ...
        (
            "mainline_live_dynlev_fix8_lock18_lb28_buf048_cd20",
            "主线 phase159：更靠近基线的保守邻域，用来校验提频是不是只能靠极端放松。",
            {
                ...
                "money_management.stake_scale.bnb_long": 1.02,
    ...
    for name, note, patch in variants:
        out.append(
            _with_meta(
                ...
                track="mainline_message_confirm_accel",
                branch=False,
                anchor_name="mainline_live_dynlev_fix8_lock18",
                playbook="mainline_message_confirm_accel",
            )
        )
    return out
...
def _new_branch_items() -> list[dict[str, Any]]:
    ...
    if btc_long is not None:
        add(
            s88._make_variant(
                btc_long,
                name="btc_breakout_long_event_lb18_atr056_adx20_cd3_s056",
                note="BTC phase159：保留事件 long，但更偏第二段延续而不是第一根冲动。",
                family="long",
                patch={
    ...
    if btc_short is not None:
        add(
            s88._make_variant(
                btc_short,
                name="btc_reclaim_short_lb18_atr055_adx20_cd3_s068",
                note="BTC phase159：保留 reclaim failure short，但不再压到极端低频。",
                family="short",
                patch={
                    "strategy_params.breakout_lookback": 18,
            ...
            playbook="btc_reclaim_short",
        )
    if btc_dual is not None:
        add(
            s88._make_variant(
                btc_dual,
                name="btc_dual_squeeze_reclaim_dynlev_fix9_cd2",
                note="BTC phase159：dual 不删，但只留 squeeze + reclaim 双轨组合。",
                family="dual",
                patch={
            ...
            playbook="btc_dual_regime",
        )

    if eth_long is not None:
        for name, note, patch in [
            (
                "eth_reclaim_long_lb11_atr043_adx16_s064",
                "ETH phase159：沿当前最强 reclaim 簇做中高预算扩圈。",
                {
            ...
            (
                "eth_reclaim_long_lb11_atr043_adx16_s068",
                "ETH phase159：激进档，只在近2年/WF 不塌时允许排前。",
                {
                    "strategy_params.breakout_lookback": 11,
                    "strategy_params.breakout_atr_buffer": 0.43,
                    "strategy_params.cooldown_bars": 4,
            ...
            (
                ...
                "ETH phase159：慢半拍确认 + 中高预算，检验 plateau 是否更稳。",
                {
            ...
            (
                ...
                "ETH phase159：更早接第二脚，用同档预算看提频是否带来更高月化。",
                {
                    "strategy_params.breakout_lookback": 12,
    ...
    if eth_short is not None:
        add(
            s88._make_variant(
                ...
                note="ETH phase159：空腿保留位，只接受回踩失败后的方向确认。",
                family="short",
                patch={
                    "strategy_params.breakout_lookback": 18,
    ...
    if sol_long is not None:
        add(
            s88._make_variant(
                ...
                note="SOL phase159：继续保留 pullback long，但只在当前最有希望邻域快刷。",
                family="long",
                patch={
        ...
        add(
            s88._make_variant(
                ...
                note="SOL phase159：尝试把 SOL 做成事件后回收，而不是只做慢回踩。",
                family="long",
                patch={
                    "sr_entries.lookback_4h": 18,
                    "sr_entries.zone_atr_mult": 0.24,
    ...
    if sol_short is not None:
        add(
            s88._make_variant(
                ...
                note="SOL phase159：guarded short 保留位，要求更快确认和更轻仓共振破位。",
                family="short",
                patch={
...
def _frontier_score(row: dict[str, Any], rows: list[dict[str, Any]], *, branch: bool) -> float:
    ...
    if sym == "eth" and fam == "long":
        if track == "eth_reclaim_cluster":
            if recent_trades >= 24 and wf_trades >= 16 and recent_pf >= 3.0 and wf_pf >= 2.0:
                bonus += 18.0
            if recent_monthly >= 0.022 and wf_monthly >= 0.026:
                bonus += 14.0
    ...
    if sym == "btc":
        ...
        if recent_trades >= 8 and wf_trades >= 8:
            bonus += 3.0
        if fam == "dual" and wf_pf >= 1.1:
            bonus += 3.0
        if target_monthly < 0.005:
            penalty += 4.0
        if wf_monthly <= 0:
            penalty += 6.0
...
def _write_report(
    ...
    lines: list[str] = []
    lines.append("Stage159 消息确认 + reclaim cluster 前沿")
    lines.append("原则：主线继续 fix8_lock18，只刷更温和的消息确认提频邻域；ETH 主攻 reclaim cluster，同时保留 squeeze 与 short；BTC/SOL 保路但不抢本轮主算力。")
    ...
    lines.append("- 消息面继续只做确认/风险层，不做单条新闻裸开仓。")
    lines.append("- reclaim 继续做第二脚确认；squeeze 只做延续补充；short 只保留回踩失败确认。")
    lines.append("- funding/OI/DVOL 只做状态切换器，不做方向按钮。")
    lines.append("")
    lines.append("=== 主线重点 ===")
    ...
    if split_rec == "keep_single_branch_terminal":
        lines.append("- 结论：继续保留 1 个 branch 终端；若后续需要隔离同一标的真实仓位，应优先用子账户，不靠多开终端硬隔离。")
    else:
    ...
    lines.append("- 这轮不动 runtime；只做主线温和提频邻域 + ETH reclaim/squeeze/short cluster + BTC/SOL 保路快刷。")

    payload = {
        "mode": "message_confirm_reclaim_cluster_frontier",
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage159 message-confirm reclaim cluster frontier")
    ...
    if ref_item is None:
        raise SystemExit("missing mainline reference for stage159")
    ...
    for item in _new_mainline_items():
        print(f"[stage159] main {item['name']}", flush=True)
    ...
    for item in _new_branch_items():
        print(f"[stage159] branch {item['name']}", flush=True)
    ...
    frontier_txt = raw / "stage159_message_confirm_reclaim_cluster_frontier_latest.txt"
    frontier_json = raw / "stage159_message_confirm_reclaim_cluster_frontier_latest.json"
    ...
    manifest_path = raw / "stage159_message_confirm_reclaim_cluster_frontier_manifest_latest.json"
