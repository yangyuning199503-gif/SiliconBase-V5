...
def _build_deepseek(okx_txt: str, stage46_txt: str, poly_txt: str) -> str:
    lines: list[str] = []
    ...
    lines.append("")
    lines.append("二、Stage46 激进实验重点")
    lines.extend(_top_lines(stage46_txt, 28) or ["- stage46_aggressive_lab_latest.txt 缺失"])
    if poly_txt.strip():
        lines.append("")
    ...
    lines.append("- 主线激进版应该先保留哪条：adx24 / adx26 / zone028 / cooldown6？")
    lines.append("- SOL long core 能否先走更激进版本，再回头保守微调？")
    lines.append("- SOL short shock 哪个版本最接近可转正？")
    lines.append("- Polymarket 作为 regime prior，阈值怎么设更稳？")
    return "\n".join(lines).rstrip() + "\n"


def main() -> None:
    ap = argparse.ArgumentParser(description="Stage46 pack outputs")
    ...
    s46_txt_p, s46_txt = _read_first([reports_raw / "stage46_aggressive_lab_latest.txt"])
    s46_json_p = reports_raw / "stage46_aggressive_lab_latest.json"
    ...
    deepseek_out.write_text(_build_deepseek(okx_txt, s46_txt, poly_txt), encoding="utf-8")

    bundle_out = downloads / "chatgpt_bundle_latest.zip"
    files_to_add: list[Path] = []
    seen = set()
    for p in [okx_p, s46_txt_p, s46_json_p if s46_json_p.exists() else None, poly_txt_p, poly_json_p if poly_json_p.exists() else None]:
