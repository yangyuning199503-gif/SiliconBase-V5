...
def _build_deepseek(okx_txt: str, main_txt: str, branch_txt: str, poly_txt: str) -> str:
    ...
    lines.append("")
    lines.append("二、Stage64 主线混合提频")
    lines.extend(_top_lines(main_txt, 18) or ["- stage64_mainline_hybrid_latest.txt 缺失"])
    lines.append("")
    lines.append("三、Stage64 ETH/SOL 广角双轨")
    lines.extend(_top_lines(branch_txt, 22) or ["- stage64_branch_hybrid_latest.txt 缺失"])
    lines.append("")
    lines.append("四、Polymarket 过滤后观察")
    lines.extend(_top_lines(poly_txt, 10) or ["- polymarket_probe_latest.txt 缺失"])
    lines.append("")
    lines.append("五、请重点给建议")
    lines.append("- 主线应优先升哪条：稳健 ref、adx28 激进版，还是 30/32 的 hybrid 版？")
    lines.append("- SOL / ETH 是否值得先升 dual 结构，还是继续把单边腿做到更稳？")
    lines.append("- Polymarket 适合只做 risk gate，还是可作为宏观事件先验层？")
    return "\n".join(lines).rstrip() + "\n"


def main() -> None:
    ap = argparse.ArgumentParser(description="Stage64 pack outputs")
    ...
    main_txt_p = reports_raw / "stage64_mainline_hybrid_latest.txt"
    main_json_p = reports_raw / "stage64_mainline_hybrid_latest.json"
    branch_txt_p = reports_raw / "stage64_branch_hybrid_latest.txt"
    branch_json_p = reports_raw / "stage64_branch_hybrid_latest.json"
    ...
    if stale:
        raise SystemExit(f"stage64 报告没有被本轮刷新：{', '.join(stale)}")
    ...
    with zipfile.ZipFile(bundle_out, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        readme = [
            ...
            f"project_dir={root}",
            f"stage64_mainline_mtime_utc={datetime.fromtimestamp(main_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            f"stage64_branch_mtime_utc={datetime.fromtimestamp(branch_txt_p.stat().st_mtime, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}",
            "",
            "包含文件：",
        ]
    ...
    must_have = {"README_SEND_TO_CHATGPT.txt", "stage64_mainline_hybrid_latest.txt", "stage64_branch_hybrid_latest.txt"}
