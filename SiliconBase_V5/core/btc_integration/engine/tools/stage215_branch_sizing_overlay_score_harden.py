...
def _candidate_eval(root: Path, row: dict[str, Any], hist: dict[str, Any], initial_equity: float) -> dict[str, Any]:
    ...
    for variant in _branch_variants():
        ...
        sign_penalty = 0.040 if float(gm_r2.get("total_return", 0.0)) < 0 else 0.0
        wf_score = float(
...
def run(project_dir: Path, out_txt: Path, out_json: Path) -> None:
    ...
    variant_leaderboard = sorted(
        ...
        key=lambda x: (x["weighted_score"], x["weighted_recent_ret"], -x["weighted_negative_recent"]),
        reverse=True,
    )
    ...
    for v in variant_leaderboard:
        if (
            ...
            and float(v.get("weighted_negative_recent", 0.0)) <= 0.45
        ):
    ...
    if recommended_variant:
        ...
        lines.append("- 原因：改成全局聚合 + 样本惩罚后，近2年收益、WF 代理增量和负收益占比最平衡。")
        lines.append("- 下一步只做一件事：把推荐 variant 同步成 branch runtime 的仓位覆盖预览，不改 entry。")
    else:
        lines.append("- 这轮不建议把任何 variant 直接同步到 branch runtime。")
        lines.append("- 原因：全局聚合后，BTC/SOL 仍拖累，且 top 方案仍带有小样本扭曲或近2年负收益。")
        lines.append("- 下一步只做一件事：继续压缩消息层到 ETH 主簇 + BTC/SOL 只保守减仓，不做激进 BOOST。")
