...
def _build_deepseek(okx_txt: str, msg_txt: str, main_txt: str, branch_txt: str, local_txt: str) -> str:
    ...
    lines.append("")
    lines.append("三、主线最终候选")
    lines.extend(_top_lines(main_txt, 10) or ["- stage65_mainline_impact_latest.txt 缺失"])
    lines.append("")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Stage69 pack outputs")
    ...
    if stale:
        raise SystemExit(f"stage69 报告没有被本轮刷新：{', '.join(stale)}")
    ...
    must_have = {"README_SEND_TO_CHATGPT.txt", "stage65_mainline_impact_latest.txt", "stage69_branch_single_leg_latest.txt"}
