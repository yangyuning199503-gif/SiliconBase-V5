...
"$PY" -m py_compile \
  ...
  "$ROOT/tools/stage140_regime_switch_risk_ladder.py" \
  "$ROOT/tools/stage139_microstructure_confluence_frontier.py" \
  "$ROOT/tools/stage138_anchor_guard_refresh.py" \
  "$ROOT/tools/stage136_regime_plateau_frontier.py" \
  "$ROOT/tools/stage90_event_alpha_matrix.py"

"$PY" -u -m tools.stage141_guarded_asymmetry_shortlist --project-dir "$ROOT"

OUT="$HOME/Downloads/stage141_guarded_asymmetry_shortlist_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage141_guarded_asymmetry_shortlist_latest.txt" \
  "$ROOT/reports/research_raw/stage141_guarded_asymmetry_shortlist_latest.json" \
  "$ROOT/reports/research_raw/stage141_guarded_asymmetry_shortlist_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage141 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage141_guarded_asymmetry_shortlist_done"
