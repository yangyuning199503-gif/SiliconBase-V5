...
cd "$ROOT"
OUT="$HOME/Downloads/stage105_joint_parallel_latest.zip"
TMP_DIR="$(mktemp -d)"
...
cp -f "$ROOT/reports/research_raw/stage105_joint_parallel_latest.txt" "$TMP_DIR/" 2>/dev/null || true
cp -f "$ROOT/reports/research_raw/stage99_mainline_frequency_push_latest.txt" "$TMP_DIR/" 2>/dev/null || true
