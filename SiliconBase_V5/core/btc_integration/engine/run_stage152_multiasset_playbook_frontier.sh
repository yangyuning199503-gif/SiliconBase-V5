...
"$PY" -u -m tools.stage152_multiasset_playbook_frontier --project-dir "$ROOT"
"$PY" -u "$ROOT/tools/stage120_event_window_frontier.py" --project-dir "$ROOT"

OUT="$HOME/Downloads/stage152_multiasset_playbook_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage152_multiasset_playbook_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage152_multiasset_playbook_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage152_multiasset_playbook_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage152 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage152_multiasset_playbook_frontier_done"
