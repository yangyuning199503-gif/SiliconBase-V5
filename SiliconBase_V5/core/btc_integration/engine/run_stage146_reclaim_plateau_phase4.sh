...
"$PY" -m py_compile \
  ...
  "$ROOT/tools/stage146_reclaim_plateau_phase4.py"

"$PY" -u -m tools.stage146_reclaim_plateau_phase4 --project-dir "$ROOT"
"$PY" -u "$ROOT/tools/stage120_event_window_frontier.py" --project-dir "$ROOT"

OUT="$HOME/Downloads/stage146_reclaim_plateau_phase4_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage146_reclaim_plateau_phase4_latest.txt" \
  "$ROOT/reports/research_raw/stage146_reclaim_plateau_phase4_latest.json" \
  "$ROOT/reports/research_raw/stage146_reclaim_plateau_phase4_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage146 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage146_reclaim_plateau_phase4_done"
