...
"$PY" -u -m tools.stage159_message_confirm_reclaim_cluster_frontier --project-dir "$ROOT"
"$PY" -u "$ROOT/tools/stage120_event_window_frontier.py" --project-dir "$ROOT"

OUT="$HOME/Downloads/stage159_message_confirm_reclaim_cluster_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
if not files:
    ...
    empty.write_text('stage159 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage159_message_confirm_reclaim_cluster_frontier_done"
