...
"$PY" -u -m tools.stage162_event_confirm_eth_pair_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage162_event_confirm_eth_pair_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage162_event_confirm_eth_pair_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage162_event_confirm_eth_pair_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage162_event_confirm_eth_pair_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage162 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage162_event_confirm_eth_pair_frontier_done"
