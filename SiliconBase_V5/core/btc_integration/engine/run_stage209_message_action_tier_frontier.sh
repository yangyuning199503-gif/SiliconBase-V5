...
OUT_ZIP="$DOWNLOADS/stage209_message_action_tier_frontier_latest.zip"
TMP_DIR="$ROOT/reports/research_raw/stage209_message_action_tier_frontier_tmp"
LOG="$TMP_DIR/stage209_full_log.txt"
STATUS="$TMP_DIR/stage209_step_status.tsv"
SUMMARY="$TMP_DIR/stage209_message_action_tier_frontier_bundle_latest.txt"
...
if ! run_step py_compile "$PY" -m py_compile \
  "$ROOT/tools/stage209_message_action_tier_frontier.py" \
...
if [ "$system_gate" = "PASS" ]; then
  ...
  if ! run_step stage209_message_action_tier_frontier "$PY" -u "$ROOT/tools/stage209_message_action_tier_frontier.py" --project-dir "$ROOT"; then
    overall_status="PARTIAL_FAIL"
  fi
else
  printf '%s\tSKIP\n' ensure_current_mainline_trades >> "$STATUS"
  printf '%s\tSKIP\n' stage209_message_action_tier_frontier >> "$STATUS"
  echo "[$(date '+%F %T')] SKIP  stage209 because system_gate=FAIL" >> "$LOG"
fi

for f in \
  "$ROOT/reports/research_raw/stage209_message_action_tier_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage209_message_action_tier_frontier_latest.json" \
...
cat > "$SUMMARY" <<EOF2
Stage209 message action tier frontier bundle

- 对外只生成 1 个文件：stage209_message_action_tier_frontier_latest.zip
- 把消息层从统一 veto 改成：pass / soft_reduce / mid_reduce / hard_block
...
- 重点不是多拦单，而是把低确定性消息改成降仓处理，减少误杀。
- 中间输出只写到 reports/research_raw；Downloads 只留 stage209 这一份 zip。
EOF2
