...
"$PY" -u -m tools.stage139_microstructure_confluence_frontier --project-dir "$ROOT"

OUT="$HOME/Downloads/stage139_microstructure_confluence_frontier_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage139_microstructure_confluence_frontier_latest.txt" \
  "$ROOT/reports/research_raw/stage139_microstructure_confluence_frontier_latest.json" \
  "$ROOT/reports/research_raw/stage139_microstructure_confluence_frontier_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage139 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage139_microstructure_confluence_frontier_done"
