...
"$PY" -u -m tools.stage138_anchor_guard_refresh --project-dir "$ROOT"

OUT="$HOME/Downloads/stage138_anchor_guard_refresh_latest.zip"
TMPDIR="$(mktemp -d)"
...
for f in \
  ...
  "$ROOT/reports/research_raw/stage138_anchor_guard_refresh_latest.txt" \
  "$ROOT/reports/research_raw/stage138_anchor_guard_refresh_latest.json" \
  "$ROOT/reports/research_raw/stage138_anchor_guard_refresh_manifest_latest.json" \
...
if not files:
    ...
    empty.write_text('stage138 export empty\n', encoding='utf-8')
    files = [empty]
if out.exists():
    out.unlink()
...
echo "stage138_anchor_guard_refresh_done"
