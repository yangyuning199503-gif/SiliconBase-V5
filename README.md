...
> 本仓库为**完整可运行的源码实现**，包括前后端源码、核心模块源码（`core/`）、Rust 硬壳扩展（`rust_core/`）和部署脚本。MIT 协议，可自由研究、修改和二次开发。

---

## 📚 相关项目

SiliconBase V5 是一个**大而全的本地 AI Agent 平台**。把功能拆开看，每个模块都有活跃的开源项目在做类似的事，但很少有项目把它们整合到一个仓库里：

| 功能 | 类似项目 |
|---|---|
| Windows 桌面自动化 / Computer-Use | [enikk](https://github.com/gtt116/enikk), [PyWinAssistant](https://github.com/a-real-ai/pywinassistant), [opendesk](https://github.com/vitalops/opendesk), [Screenhand](https://github.com/manushi4/screenhand), [desktop-touch-mcp](https://github.com/Harusame64/desktop-touch-mcp) |
| 持久记忆 / 记忆架构 | [mem0](https://github.com/mem0ai/mem0), [Letta](https://github.com/letta-ai/letta), [Neuroca](https://github.com/Modern-Prometheus-AI/Neuroca) |
| 意识 / 自主 Agent 框架 | [project-prometheus](https://github.com/yuvaraj030/project-prometheus), [AutoGPT](https://github.com/Significant-Gravitas/AutoGPT) |
| 聚合列表 | [Awesome-AI-Agents](https://github.com/Jenqyang/Awesome-AI-Agents), [Awesome-Memory-for-Agents](https://github.com/TsinghuaC3I/Awesome-Memory-for-Agents) |

> 这些项目通常使用英文关键词（`computer-use agent`、`gui automation`、`persistent memory`），因此用中文搜索 GitHub 时不易发现。添加英文简介和 topics 后，SiliconBase V5 更容易被同类用户找到。
...
这是**个人研究开源项目**，代码可以直接跑起来，但还在持续打磨中。
...
```text
SiliconBase_V5/
├── SiliconBase_V5/          # 后端源码（Python / FastAPI）
│   ├── api/                 # HTTP API 与 WebSocket 入口
│   ├── core/                # 核心模块源码
│   │   ├── agent/           # AgentLoop 主循环
│   │   ├── consciousness/   # 意识线程与四层架构
│   │   ├── memory/          # L1-L5 记忆系统
│   │   ├── tool/            # 工具管理器
│   │   ├── vision/          # 视觉感知
│   │   └── btc_integration/ # AI 交易子系统
│   ├── frontend/            # 前端源码（React / TypeScript / Vite）
│   ├── rust_core/           # Rust / PyO3 硬壳扩展源码
│   └── scripts/             # 初始化与部署脚本
├── docs/                    # 架构与设计文档
├── README.md                # 本文件
└── LICENSE                  # MIT 许可证
...
**代码统计**：约 15 万行源码，主要语言 Python（后端）+ TypeScript（前端），少量 Rust（核心扩展）。
